...
function db(): PDO
{
    ...
    if (DB_DRIVER === 'mysql') {
        ...
        foreach ($hosts as $host) {
            $dsns = [];
            $dsns[] = sprintf('mysql:host=%s;port=%s;dbname=%s;charset=%s', $host, DB_PORT, DB_NAME, DB_CHARSET);
            if ($host === 'localhost') {
                // Allow local socket resolution on shared hosting.
                $dsns[] = sprintf('mysql:host=%s;dbname=%s;charset=%s', $host, DB_NAME, DB_CHARSET);
            }
        ...
        if (!$pdo instanceof PDO) {
            $message = $lastError instanceof Throwable ? $lastError->getMessage() : 'Unknown MySQL connection error.';
            throw new RuntimeException($message);
        }
    } else {
        if (!extension_loaded('pdo_sqlite')) {
            throw new RuntimeException('pdo_sqlite extension is not enabled on this server.');
        }
        $dir = dirname(DB_PATH);
        if (!is_dir($dir)) {
            mkdir($dir, 0775, true);
    ...
    if (DB_DRIVER === 'mysql') {
        ...
        $pdo->exec(
            'CREATE TABLE IF NOT EXISTS page_view_hits (
                ...
                page_slug TEXT NOT NULL DEFAULT \'\',
                product_id INTEGER NOT NULL DEFAULT 0,
                country_code TEXT NOT NULL DEFAULT \'UNK\',
        ...
        $pdo->exec('CREATE UNIQUE INDEX IF NOT EXISTS idx_page_view_daily ON page_views(view_date, path, page_type, page_slug, product_id)');
        $pdo->exec('CREATE INDEX IF NOT EXISTS idx_page_views_type ON page_views(page_type)');
        $pdo->exec('CREATE INDEX IF NOT EXISTS idx_page_views_product ON page_views(product_id)');
        $pdo->exec('CREATE INDEX IF NOT EXISTS idx_page_views_date ON page_views(view_date)');
        $pdo->exec('CREATE INDEX IF NOT EXISTS idx_hits_date ON page_view_hits(view_date)');
        $pdo->exec('CREATE INDEX IF NOT EXISTS idx_hits_country ON page_view_hits(country_code)');
        $pdo->exec('CREATE INDEX IF NOT EXISTS idx_hits_source ON page_view_hits(source_type)');
        $pdo->exec('CREATE INDEX IF NOT EXISTS idx_hits_referrer ON page_view_hits(referrer_host)');
        $pdo->exec('CREATE INDEX IF NOT EXISTS idx_hits_path ON page_view_hits(path)');
        $pdo->exec('CREATE INDEX IF NOT EXISTS idx_hits_product ON page_view_hits(product_id)');
        $pdo->exec('CREATE INDEX IF NOT EXISTS idx_outbound_date ON outbound_clicks(click_date)');
        $pdo->exec('CREATE INDEX IF NOT EXISTS idx_outbound_product ON outbound_clicks(product_id)');
        $pdo->exec('CREATE INDEX IF NOT EXISTS idx_outbound_source ON outbound_clicks(source_type)');
        $pdo->exec('CREATE INDEX IF NOT EXISTS idx_outbound_country ON outbound_clicks(country_code)');
        $pdo->exec('CREATE INDEX IF NOT EXISTS idx_outbound_from_path ON outbound_clicks(from_path)');
